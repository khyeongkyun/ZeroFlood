...
# NOTE: Translate timestamp metadata into the readable form.
from datetime import datetime, timezone
def timestamp_to_str(timestamp):
    return datetime.fromtimestamp(int(timestamp) / 1e9, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
...
def build_wds_dataset(
    ...
    print(f"FLAG: dataset {dataset.shape}")
